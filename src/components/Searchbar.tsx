...
export default function Searchbar({ projects }: { projects: Project[] }) {
  ...
  const handleSearch = useDebouncedCallback((searchQuery: string) => {
    ...
    if (searchQuery) {
      params.set("search", searchQuery); // Adds ?search=term to URL
    } else {
      params.delete("search"); // Removes search parameter if input is empty
    }

    // 2. Updates the URL path without reloading the whole browser tab
